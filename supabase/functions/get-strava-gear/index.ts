...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Edge Function get-strava-gear called with token:", 
      access_token ? `${access_token.substring(0, 5)}...` : "token absent");
    ...
    try {
      ...
      console.log("Parsed Strava API response:", {
        ...
        ok: response.ok,
        id: data.id || "absent",
        username: data.username || "absent",
        firstname: data.firstname || "absent",
        lastname: data.lastname || "absent",
        has_bikes: Boolean(data.bikes) && Array.isArray(data.bikes),
        bikes_count: data.bikes ? data.bikes.length : 0
      });
    ...
    // Extract only the bikes array from athlete data
    const result = { gear: data.bikes || [] };
    console.log(`Returning ${result.gear.length} bikes`);
